...
namespace ESPORT.Forme
{
    public partial class TimoviForma : Form
    {
        ...
        public void popuniPodacima()
        {
            ...
            List<TimDTO.TimPregled> podaci =
                DTOManager.vratiSveTimove();

            foreach (TimDTO.TimPregled t in podaci)
            {
                ListViewItem item =
                    new ListViewItem(t.TimId.ToString());

                item.SubItems.Add(t.Naziv);
                item.SubItems.Add(t.Igra);
                item.SubItems.Add(
                    t.DatumOsnivanja.HasValue
                        ? t.DatumOsnivanja.Value.ToString("dd.MM.yyyy")
                        : "");
        ...
        private void izmenitimbtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "Molimo vas da izaberete tim koji želite da izmenite!",
                    "Upozorenje",
            ...
            int idTima =
                int.Parse(
                    listViewTimovi.SelectedItems[0]
                        .SubItems[0].Text);

            IzmeniTimForma forma =
                new IzmeniTimForma(idTima);
        ...
        private void obrisitimbtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "Molimo vas da izaberete tim koji želite da obrišete!",
                    "Upozorenje",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
            ...
            DialogResult result =
                MessageBox.Show(
                    ...
                    "Biće obrisani i svi podaci vezani za ovaj tim " +
                    "(ugovori, transferi, pozajmice, angažmani, učešća i statistike).",
                    "Potvrda brisanja",
        ...
        private void btnAngazmaniTrenera_Click(
            object sender,
            EventArgs e)
        {
            AngazmanTreneraForma forma =
                new AngazmanTreneraForma();

            forma.ShowDialog();
        }

        private void btnAngazmaniZaposlenih_Click(
            object sender,
            EventArgs e)
        {
            AngazmanZaposlenihForma forma =
                new AngazmanZaposlenihForma();
